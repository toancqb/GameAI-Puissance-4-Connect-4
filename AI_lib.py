...
def max_eval(mn, result, cp_NB_PLAYED, depth, prev_coord):
	if (depth == 0):
	...
	for y in range(TY):
		...
		if is_winning_pos(new_mn, coord[0], coord[1]):
			...
			result = 0
		m = min_eval(new_mn, result, cp_NB_PLAYED + 1, depth-1, coord)
		if (m[0] > max_v):
			max_v, px, py = m[0], m[1], m[2]

	return (max_v, px, py)

def min_eval(mn, result, cp_NB_PLAYED, depth, prev_coord):
	if (depth == 0):
		sign = -1
	...
	for y in range(TY):
		...
		if is_winning_pos(new_mn, coord[0], coord[1]):
			...
			result = 0
		m = max_eval(new_mn, result, cp_NB_PLAYED + 1, depth-1, coord)
		if (m[0] < min_v):
...
def minimax_AI(ar, NB_PLAYED, depth):
	...
	(m, max_i, max_j) = max_eval(mn, 2, cp_NB_PLAYED, depth, ())
	return (m, max_j)
...
def score_pos2(ar, coord, p):
	...
	for i in range(-3, 1):
		if (y+i >= 0 and y+i+3 < TY):
			score += idm3(p, ar[x][y+i], ar[x][y+i+1], ar[x][y+i+2], ar[x][y+i+3])

		if (x+i >= 0 and x+i+3 < TX):
			score += idm3(p, ar[x+i][y], ar[x+i+1][y], ar[x+i+2][y], ar[x+i+3][y])

		if (x+i >= 0 and x+i+3 < TX and y+i >= 0 and y+i+3 < TY):
			score += idm3(p, ar[x+i][y+i], ar[x+i+1][y+i+1], ar[x+i+2][y+i+2], ar[x+i+3][y+i+3])
	...
	for i in ch:
		if (check_range(x+i[0],0,TX-1) and check_range(y+i[1],0,TY-1)
			and check_range(x+i[0]-3, 0, TX-1) and check_range(y+i[1]+3, 0, TY-1)):
			score += idm3(p, ar[x+i[0]][y+i[1]], ar[x+i[0]-1][y+i[1]+1]
				, ar[x+i[0]-2][y+i[1]+2], ar[x+i[0]-3][y+i[1]+3])

	return score

def score_ar2(ar, player):
	mn = cp_arena(ar)
	max_point = NEG_INF
	tmp, px, py = 0, None, None
	for y in range(TY):
		...
		if coord == ():
			continue
		tmp = score_pos2(new_mn, coord, player)
		if (tmp > max_point):
			max_point = tmp
			px = coord[0]
			py = coord[1]
...
def minimax(ar, player, NB_PLAYED, depth, result, a, b):
	if result == 1:
		return (1000000, None)
	elif result == 0:
		return (0, None)
	...
	if player == 1:
		...
		for y in range(TY):
			...
			if is_winning_pos(cp_ar, coord[0], coord[1]):
				...
				result = 0
			score = minimax(cp_ar, 0, NB_PLAYED+1, depth-1, result, a,b)
			if score[0] > max_v:
				max_v = score[0]
				py = coord[1]
			if a < max_v:
		...
		for y in range(TY):
			...
			if is_winning_pos(cp_ar, coord[0], coord[1]):
				...
				result = 0
			score = minimax(cp_ar, 1, NB_PLAYED+1, depth-1, result, a,b)
			if score[0] < min_v:
				min_v = score[0]
				py = coord[1]
			if b > min_v:
	...
	while (NB_PLAYED < TX*TY):
		...
		if (t == 0):
			...
			if mode == 1:
				...
				y = minimax(arena, 1, NB_PLAYED, depth, 2, NEG_INF, INF)[1]
	...
	while (NB_PLAYED < TX*TY):
		if (t == 0):
			if mode:
				...
				m = minimax(arena, 0, NB_PLAYED, depth1, 2, NEG_INF, INF)
				y = m[1]
		else:
			m = minimax(arena, 1, NB_PLAYED, depth2, 2, NEG_INF, INF)
			y = m[1]
		clear()
